$sourcefile = "e:\Scripts\Useralias.csv" #file to read. needs to include full path
...
if ($logging)
{
	$filename = "$($reportpath)update-user-aliases-$(get-date -f yyyy-MM-dd_hhmmss).txt"
	$stream = [System.IO.StreamWriter] "$($filename)"
	$stream.WriteLine("Adding aliases to accounts!")
	$stream.WriteLine("Starting on $(get-date -f yyyy-MM-dd_hhmmss)")
}

foreach ($entry in $list)
  {
	"ALIAS $($aliascount): Adding $($entry.aliasname) to account $($entry.Username)"
	$stream.WriteLine("ALIAS $($aliascount): Adding $($entry.aliasname) to account $($entry.Username)")
	
	$useremail = $($entry.Username) -replace "\'","``'"

	$aliasemail = $($entry.aliasname) -replace "\'","``'"
	
	$cmd = "$($gampath)gam.exe create alias `"$($aliasemail)`" user `"$($useremail)`" 2>&1"
